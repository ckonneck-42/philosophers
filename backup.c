...
/*   Updated: 2024/09/13 11:02:23 by ckonneck         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*observer(void *arg)
{
	...
	while (1)
	{
    	...
    	if (*philos->shared_dead == 1 || philos->meals_eaten == philos->target) 
		{
        	...
        	return NULL;  // Stop this observer thread
   		}
		pthread_mutex_unlock(philos->shared_dead_mutex);
		// printf("time to die %f\n", philos->time_to_die);
		if(philos->timesinceeaten >= philos->time_to_die)
		{
			// printf("philo %d time since eaten %.3lld\n",num, philos->timesinceeaten);
			if (*philos->shared_dead == 1 || philos->meals_eaten == philos->target) 
				return (NULL);
			pthread_mutex_lock(philos->shared_dead_mutex);
...
int	main(int argc, char **argv)
{
    ...
    *shared_dead = 0;
	...
	while (i < number_of_philosophers)
	{
		...
		pthread_create(&philos->observer_threads[i], NULL, observer, data);
		pthread_create(&philos->threads[i], NULL, threadfunction, data);
		i++;
	}
	i = 0;
	while (i < number_of_philosophers)
...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/06 10:55:22 by ckonneck          #+#    #+#             */
/*   Updated: 2024/09/13 09:52:27 by ckonneck         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
